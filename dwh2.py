...
class CreateTestSchemaWindow(Tkinter.Toplevel):
    ...
    def initWindow(CrSchemaWindow, SID, passwd):
        ...
        Ratio1 = Tkinter.Radiobutton(CrSchemaWindow, text='1: Disk space needed 11MB', variable=CrSchemaWindow.RatioVar,
                                     value=1)
        Ratio2 = Tkinter.Radiobutton(CrSchemaWindow, text='2: Disk space needed 22MB', variable=CrSchemaWindow.RatioVar,
                                     value=2)
        Ratio3 = Tkinter.Radiobutton(CrSchemaWindow, text='3: Disk space needed 43MB', variable=CrSchemaWindow.RatioVar,
                                     value=3)
        Ratio4 = Tkinter.Radiobutton(CrSchemaWindow, text='4: Disk space needed 88MB', variable=CrSchemaWindow.RatioVar,
                                     value=4)
        Ratio5 = Tkinter.Radiobutton(CrSchemaWindow, text='5: Disk space needed 176MB',
                                     variable=CrSchemaWindow.RatioVar, value=5)
        Ratio6 = Tkinter.Radiobutton(CrSchemaWindow, text='6: Disk space needed 344MB',
                                     variable=CrSchemaWindow.RatioVar, value=6)
        Ratio7 = Tkinter.Radiobutton(CrSchemaWindow, text='7: Disk space needed 680MB',
                                     variable=CrSchemaWindow.RatioVar, value=7)
        Ratio8 = Tkinter.Radiobutton(CrSchemaWindow, text='8: Disk space needed 1.4GB',
                                     variable=CrSchemaWindow.RatioVar, value=8)
        Ratio9 = Tkinter.Radiobutton(CrSchemaWindow, text='9: Disk space needed 2.7GB',
                                     variable=CrSchemaWindow.RatioVar, value=9)
        Ratio10 = Tkinter.Radiobutton(CrSchemaWindow, text='10: Disk space needed 5.5GB',
                                      variable=CrSchemaWindow.RatioVar, value=10)
...
class CreateAProposWindow(Tkinter.Toplevel):
    def __init__(AProposWindow):
        ...
        """
        AProposWindow.initWindow()

    def initWindow(AProposWindow):
        ...
        while GraphWindow.LoopGraphVar == 0:
            ...
            if error_con != 1:
                ...
                for result in curRampUp:
                    ...
                    if int(result[0]) > 0:
                        curValue.execute(
                            'select elapsed from (select seq, elapsed from dwhstat) where seq>(select max(seq) - 20 from dwhstat) order by seq')
                        for y in curValue:
                            x0 = pos * 20 + 10
...
class simpleapp_tk(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        self.LabelExecTime.grid(column=1, row=19)
        ...
        self.LabelNbQueries.grid(column=1, row=20)
        ...
        buttonQuit.grid(column=1, row=21, sticky=S)
        ...
        self.ButtonCreateSchema.grid(column=2, row=6, columnspan=2)
    ...
    def OnValidate(self, action, index, value_if_allowed, prior_value, text, validation_type, trigger_type,
                   widget_name):
        """
    ...
    def InitTableStat(self):
        ...
        if error_con != 1:
            ...
            try:
                ...
                if error.code == 942:
                    self.labelVariable.set(
                        self.entryConnectStringVariable.get() + ": Test schema does not exist. Create it first!")
            cur.close()
            con.close()
    ...
    def ExecTime(self):
        ...
        if error_con != 1:
            ...
            for result in curRampUp:
                ...
                if int(result[0]) > 10:
                    curExecTime.execute(
                        'select (sum(elapsed))/10 from (select seq, elapsed from dwhstat) where seq>(select max(seq) - 10 from dwhstat)')
